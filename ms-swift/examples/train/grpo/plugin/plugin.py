...
# ============================================================================
# GUI-R1 Reward Functions
# Adapted from verl/verl/utils/reward_score/gui_r1.py
# ============================================================================
...
class GuiR1FormatReward(ORM):
    """
    Format reward for GUI-R1 dataset.
    Checks if the prediction follows the expected format:
    <think></think><answer>[{'action': ..., 'point': [...], 'input_text': ...}]</answer>
    """

    def __call__(self, completions, **kwargs) -> List[float]:
        """
        Check if completions follow the format: <think></think><answer></answer>
        ...
        Returns:
            list[float]: Format reward scores (0.0 or 1.0)
        """
        rewards = []
        for predict_str in completions:
            reward = self._check_format(predict_str)
            rewards.append(reward)
        return rewards

    @staticmethod
    def _check_format(predict_str: str) -> float:
        """
        ...
        try:
            actions = eval(answer_content)  # Parse <answer> content as JSON
...
class GuiR1AccuracyReward(ORM):
    """
    Accuracy reward for GUI-R1 dataset.
    Compares actions and parameters between prediction and ground truth.
    """

    def __call__(self, completions, solution, **kwargs) -> List[float]:
        """
        Compare actions and parameters between predictions and ground truth.

        Args:
            completions (list[str]): Generated outputs with <think> and <answer> tags
            solution (list[dict]): Ground truth solutions containing 'action', 'bbox', 'input_text'

        Returns:
            list[float]: Accuracy reward scores (0.0 or 1.0)
        """
        rewards = []
        for predict_str, sol in zip(completions, solution):
            try:
                # Extract ground truth from solution dict
                gt_action = sol.get('action', '')
                gt_bbox = sol.get('bbox', [-100, -100])
                gt_input_text = sol.get('input_text', 'no input text')

                reward = self._compute_accuracy(predict_str, gt_action, gt_bbox, gt_input_text)
                rewards.append(reward)
            except Exception as e:
    ...
    @staticmethod
    def _compute_accuracy(predict_str: str, gt_action: str, gt_bbox: list, gt_input_text: str) -> float:
        """
        ...
        Args:
            ...
            gt_bbox: Ground truth bounding box [x, y] or [x1, y1, x2, y2]
        ...
        gt_input_text = gt_input_text if gt_input_text else 'no input text'
...
class GuiR1CombinedReward(ORM):
    """
    Combined reward for GUI-R1 dataset.
    Combines format checking and accuracy checking with configurable weights.
    ...
    Or use individual reward components:
        --reward_funcs gui_r1_format gui_r1_accuracy
    """

    def __init__(self, format_weight: float = 0.2, accuracy_weight: float = 0.8):
        """
        ...
        self.format_reward = GuiR1FormatReward()
        self.accuracy_reward = GuiR1AccuracyReward()

    def __call__(self, completions, solution, **kwargs) -> List[float]:
        """
        Compute combined reward scores.
        ...
        Returns:
            list[float]: Combined reward scores (weighted sum of format and accuracy)
        """
        # Compute format scores
        format_scores = self.format_reward(completions)

        # Compute accuracy scores
        accuracy_scores = self.accuracy_reward(completions, solution=solution, **kwargs)

        # Combine scores
        combined_scores = [
            self.accuracy_weight * acc + self.format_weight * fmt
            for acc, fmt in zip(accuracy_scores, format_scores)
        ]

        return combined_scores


# Register GUI-R1 reward functions
orms['gui_r1_format'] = GuiR1FormatReward
orms['gui_r1_accuracy'] = GuiR1AccuracyReward
orms['gui_r1_combined'] = GuiR1CombinedReward
